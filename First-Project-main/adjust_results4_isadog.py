...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    # Read names of dogs from file with striping
    # Read names of dogs from file with striping
    with open(dogfile, 'r') as file:
        dog_names = file.readlines()
        dog_names = [name.strip() for name in dog_names]

    # Convert dog_names to a set for faster lookup
    dog_names_set = set(dog_names)

    # Looping inside each key & value pair in the results_dic
    for value in results_dic.values():
        # Check if the pet image label is a dog
        pet_image_is_dog = 1 if value[0] in dog_names else 0
        ...
        value.extend([pet_image_is_dog, classifier_label_is_dog])
